...
vector<LexemString> BBCoreLexemParser::preprocessor(list<LexemString> lexems){
...
BBTokensAnalyzer_ptr BBCoreLexemParser::tokensAnalyzerFromString(string str,ErrorPool_ptr errorPool) {
    list<LexemString> lexems = this->lexAnalyzer.lexemsFromSting (str);
    vector<LexemString> preprocessorLexems = this->preprocessor (lexems);
    ...
    creater->errorPool = this->errorPool;
    for (list<SyntacticResultObject_ptr>::iterator obj = objects.begin();
         obj != objects.end();
         ++obj){
        (*obj)->getData(creater);
...
BBCoreLexemParser::BBCoreLexemParser(ErrorPool_ptr errorPool):errorPool(errorPool),lexAnalyzer(errorPool),syntacticAnalyzer(errorPool) {
    debugeMod = false;
    ...
    this->syntacticAnalyzer = BBSyntacticAnalyzer (errorPool);
